    ...
    with initialize(version_base=None, config_path="../../conf"):
        ...
        grid = [
            # 1. Main CoT Faithfulness Sweep
            {
                ...
                "prompts": [
                    "chain_of_thought", "direct_answer", "sycophantic", 
                    "adversarial", "uncertainty", "expert_persona", 
                    "few_shot", "contrarian", "arrogance", 
                    "simple", "socratic", "no_instruction", "radiology"
                ]
            },
            # 2. Specialized Radiology Experiment
            {
                "experiment": "radiology",
                "datasets": ["radiology"],
                "prompts": ["radiology"]
            }
        ]
            ...
            for dataset_name in group["datasets"]:
                for prompt_name in group["prompts"]:
                    ...
                    try:
                        ...
                        # Basic metrics print
                        metrics_str = ", ".join([f"{k}={v:.2f}" if isinstance(v, float) else f"{k}={v}" for k, v in result.metrics.items()])
