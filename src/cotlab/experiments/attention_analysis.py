...
class AttentionAnalysisExperiment(BaseExperiment):
    ...
    def run(
        ...
        for layer_idx in self.target_layers:
            ...
            print(f"L{layer_idx:<7} | {avg_entropy:<12.4f} | H{min_head} ({min_entropy:.3f}) | H{max_head} ({max_entropy:.3f})")
        ...
        all_entropies = [layer_entropies[l]["avg_entropy"] for l in layer_entropies]
        ...
        if layer_entropies:
            most_focused_layer = min(layer_entropies.keys(), 
                                     key=lambda l: layer_entropies[l]["avg_entropy"])
        ...
        if "most_focused_layer" in metrics:
            print(f"Most focused layer: L{metrics['most_focused_layer']} (entropy: {metrics['most_focused_entropy']:.4f})")
